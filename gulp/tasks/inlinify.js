...
// consts
const PLUGIN_NAME = 'inlinify';

function inlinify(readFrom, writeTo, onError, onCompleted) {
    ...
    function readFromBuffer() {
        ...
        var rx = {
            xref: /^\s*\/\/\s*@inlinify:include\s+/i,
            write: /^\s*\/\/\s*@inlinify:write\s/i,
            rootPath: /^\s*\/\/\s*@inlinify:rootpath\s+/i,
            startOmit: /^\s*\/\/\s*@inlinify:startomit($|\s.*)/i,
            endOmit: /^\s*\/\/\s*@inlinify:endomit($|\s.*)/i,
            export: /^\s*export\s+/,
            import: /^\s*import\s+/
        };
        ...
        if(!ignoringLines && rx.xref.test(line)){
            ...
            inlinify(line, writeTo, errorHandler, finishBufferRead);
        }
...
function main() {
  ...
  var stream = through.obj(function(file, enc, cb) {
    ...
    if (file.isStream()) {
      ...
      var outStream = inlinify(file.contents, null, function error(err){
